...
public class AA6_14 {
	...
	static void buscar(String[] diccionario) {
		...
		for (int i = 0; i < diccionario.length && !salir; i++) {
			...
			// Cogemos la parte izquierda para compararla con el nombre que nos ha dado el usuario
			ini = actual.substring(0, indice);
			// Solo en el caso que los nombres sean iguales salimos del bucle y terminamos
			if (nombre.equals(ini)) {
				salir = true;
			}
		}
